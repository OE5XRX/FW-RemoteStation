...
// stays stable vs the success path. The caller passes the same schema the success path uses
// (describe -> 2, status -> 1). The fallback is short enough that it can never itself
// overflow. moduleId is a registered literal, so it needs no JSON escaping.
void emit_too_long_frame(const struct shell *sh, const char *frame, int schema, const char *module) {
  shell_print(sh, "%s {\"schema\":%d,\"module\":\"%s\",\"error\":\"too_long\"}", frame, schema, module);
}
...
int cmd_module(const struct shell *sh, size_t argc, char **argv) {
  ...
  if (!strcmp(op, "describe")) {
    ...
    m->describe(w);
    if (w.truncated()) {
      emit_too_long_frame(sh, "MODULE-DESCRIBE", 2, m->moduleId());
      return 0;
    }
    shell_print(sh, "%s", w.c_str());
    return 0;
  ...
  if (!strcmp(op, "status")) {
    ...
    m->snapshot(w);
    if (w.truncated()) {
      emit_too_long_frame(sh, "MODULE-STATUS", 1, m->moduleId());
      return 0;
    }
    shell_print(sh, "%s", w.c_str());
    return 0;
