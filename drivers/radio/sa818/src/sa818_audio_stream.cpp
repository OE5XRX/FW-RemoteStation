...
static void audio_stream_work_handler(struct k_work *work) {
  ...
  if (ctx->callbacks.tx_request && data->audio_tx_enabled) {
    size_t bytes = ctx->callbacks.tx_request(ctx->dev, ctx->tx_buffer, 
  ...
  if (ctx->callbacks.rx_data && data->audio_rx_enabled) {
    ...
    if (ret == 0) {
      ...
      ctx->callbacks.rx_data(ctx->dev, ctx->rx_buffer, SA818_AUDIO_SAMPLE_SIZE,
                             ctx->callbacks.user_data);
    }
  }
...
sa818_result sa818_audio_stream_start(const struct device *dev,
  ...
  LOG_INF("Audio streaming started: %u Hz, %u-bit, %u ch",
