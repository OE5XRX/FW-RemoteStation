...
static int cmd_sa818_test_sweep(const struct shell *shell, size_t argc, char **argv) {
  if (argc < 3) {
    ...
    shell_error(shell, "  amplitude: 0-255 (default: 128)");
    shell_error(shell, "  Note: Sweep runs continuously (loops endlessly)");
    shell_error(shell, "example: sa818 test sweep 100 3000           (10 sec sweep, loops forever)");
    shell_error(shell, "example: sa818 test sweep 500 2000 5000      (5 sec sweep cycles)");
    shell_error(shell, "example: sa818 test sweep 100 3000 8000 200  (8 sec sweep at 78%% amp)");
    return -EINVAL;
  }
  ...
  if (ret != SA818_OK) {
    shell_error(shell, "Failed to generate sweep: %d", ret);
    return ret;
  }

  shell_print(shell, "Continuous frequency sweep started: %d Hz -> %d Hz, cycling every %u ms at amplitude %u", start_freq, end_freq, duration_ms, amplitude);
...
SHELL_STATIC_SUBCMD_SET_CREATE(
    ...
    SHELL_CMD(sweep, NULL, "Frequency sweep", cmd_sa818_test_sweep),
