# SA818 USB Audio Bridge

USB Audio Class 2 (UAC2) Integration für den SA818 Radio Driver.

## Übersicht

Die USB Audio Bridge ermöglicht bidirektionales Audio-Streaming zwischen einem USB-Host (PC, Raspberry Pi, etc.) und dem SA818 Radio-Modul:

```
USB Host (PC) ←→ UAC2 ←→ Ring Buffer ←→ DAC/ADC ←→ SA818 Radio
```
...
```kconfig
CONFIG_USBD_AUDIO2_CLASS=y  # UAC2 aktivieren
...
int main(void) {
    ...
    // USB Audio Bridge initialisieren
    sa818_usb_audio_init(sa818, uac2);
    sa818_usb_audio_enable(sa818);
