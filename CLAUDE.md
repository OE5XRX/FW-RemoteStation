...
- **`std::optional` und `std::variant` bewusst nutzen, wo sie die Absicht klarer machen** — nicht erzwingen, sondern abwägen, ob es passt. `std::optional` statt Sentinel-Werten/Null-Flags für „Wert oder nichts"; `std::variant` statt tagged unions / Typ-Enums für „einer von mehreren Typen". Beide sind allokationsfrei (stack) und passen zur no-alloc-Regel.
